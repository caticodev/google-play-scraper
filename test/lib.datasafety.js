...
import { assertValidUrl as assertValidUrl$0 } from "./common.js";
'use strict';
const assert = { assert: assert$0 }.assert;
const assertValidUrl = { assertValidUrl: assertValidUrl$0 }.assertValidUrl;
...
describe('Data Safety method', () => {
    it('should return arrays of data shared, data collected, security practices and a privacy url', () => gplay.datasafety({ appId: 'com.sgn.pandapop.gp' })
        .then((dataSafety) => {
    ...
    it('should return a valid shared and collected data object', () => gplay.datasafety({ appId: 'com.sgn.pandapop.gp' })
        .then((dataSafety) => {
        dataSafety.sharedData.map(assertValidDataSafetyObject());
        dataSafety.collectedData.map(assertValidDataSafetyObject());
    }));
    it('should return a valid security practices object', () => gplay.datasafety({ appId: 'com.sgn.pandapop.gp' })
        .then((dataSafety) => {
    ...
    it('should return empty return for non existing app', () => gplay.datasafety({ appId: 'app.foo.bar' })
        .then((dataSafety) => {
